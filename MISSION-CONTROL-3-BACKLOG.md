# Mission Control 3.0 backlog

## AI learning
- Learn from manual text corrections and accepted image choices.
- Explain learned preferences and allow reset/override.

## Automated quality missions
- Guest-perspective completeness review.
- SEO coverage and structured-data review.
- Image coverage by season, audience and campaign.
- Chatbot gap analysis from real anonymised questions.

## Direct deployment
- Replace manual GitHub file upload with a secure server-side publish action.
- Preserve approval, manifest, rollback and live verification states.
- Add rollback to a previous confirmed release.

## Future operating areas
- Booking and guest communication.
- Maintenance planning.
- Smart-home status.
- Privacy-safe analytics and conversion insights.


## Strategic backlog added in v2026.07.24.94

- Product naming: define a distinct name for the administration product, separate from Casa Amar.
- Multi-user foundation: login, invitations, roles and permissions.
- Suggested initial roles: Owner/Publisher, Content Creator and Reviewer.
- Audit history: attribute changes and approvals to users.
- Keep the present experience single-user until authentication is intentionally implemented.


## Timestamp principle confirmed in v2026.07.24.94

- Primary user timestamps always represent a confirmed completed state.
- Platform: first successful consistency confirmation for the current version.
- Content: second successful public live match.
- Build/start/upload timestamps remain available only in activity logs or technical details.


## Release lifecycle clarification implemented in v2026.07.24.94

- Platform updates and content releases are separate lifecycles.
- Content monitoring starts only after an explicit user content-package action.
- Old sessions are retained as historical receipts and cannot restart monitoring.
- Idle state shows status and one next-step CTA.


## Canonical route correction implemented in v2026.07.24.94

- Every historical mission-control-vXX URL resolves to the current canonical Mission Control.
- Browser URL is normalized to the current versioned route.
- “Opdater platformen” navigates to the new canonical route instead of reloading the old page.
