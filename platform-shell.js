...
function casaMigrateLegacyWorkspace(){
 ...
 const backup={created_at:new Date().toISOString(),platform_version:"v2026.07.24.149",items:{}};
...
async function fetchPlatformManifest(){
 const response=await fetch(`/platform-manifest.json?v=20260724.149&_=${Date.now()}`,{cache:"no-store",headers:{"cache-control":"no-cache"}});
...
async function fetchPlatformIdentityPair(){
 const [manifest,metaResponse]=await Promise.all([
  ...
  fetch(`/api/platform-meta?v=20260724.149&_=${Date.now()}`,{cache:"no-store",headers:{"cache-control":"no-cache"}})
 ]);
...
fetch(`/content-release.json?v=20260724.149&_=${Date.now()}`,{cache:"no-store"}).then(r=>r.json()).then(data=>{
