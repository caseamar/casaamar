(()=>{
 const VERSION='1.1.0',KEY='casa-domain-context-v1';
 ...
 async function load(){
  ...
  state.packs=packs; const saved=localStorage.getItem(KEY); state.model=saved?JSON.parse(saved):model;
  state.model.clarifications=state.model.clarifications||[]; return state.model;
 }
 ...
 function upsertFact(subject,predicate,object,status='verified',source='human-verification'){
  const facts=state.model.facts||(state.model.facts=[]); const found=facts.find(f=>f.subject===subject&&f.predicate===predicate);
  const next={subject,predicate,object,status,source,verifiedAt:new Date().toISOString()};
  if(found)Object.assign(found,next);else facts.push(next); return next;
 }
 function addFollowup(def){const list=state.model.clarifications||(state.model.clarifications=[]);if(!list.some(x=>x.id===def.id))list.push({...def,status:'open',createdAt:new Date().toISOString()});}
 function answerClarification(id,answer,value=''){
  ...
  if(a==='yes'){
   if(q.subject&&q.predicate)upsertFact(q.subject,q.predicate,q.yesValue??true,'verified');
   q.status='resolved'; message=`${q.question.replace(/\?$/,'')} er bekræftet.`;
  }else if(a==='no'){
   q.status='resolved';
   if(q.noFollowup==='patio-floor-location')addFollowup({id:'patio-floor-location',type:'choice_or_text',question:'Hvor ligger patioen?',why:'AI skal kende den korrekte placering for at kunne skelne områder og beskrive huset præcist.',subject:'area.patio',predicate:'located_on_floor',options:[{value:'first-floor',label:'1. sal'},{value:'second-floor',label:'2. sal'},{value:'other',label:'Andet'}]});
   if(q.noFollowup==='roof-sharing')addFollowup({id:'roof-sharing',type:'choice_or_text',question:'Hvem deles tagterrassen med?',why:'AI skal kende adgangsforholdene, før den bruger ord som privat eller fælles.',subject:'area.roof-terrace',predicate:'shared_with',options:[{value:'other-homes',label:'Andre boliger'},{value:'community',label:'Fællesområde'},{value:'other',label:'Andet'}]});
   ...
   upsertFact(q.subject,q.predicate,clean,'verified'); q.status='resolved'; q.value=clean; message=`Registreret: ${clean}.`;
 ...
 window.CasaDomainIntelligence={VERSION,load,saveDescription,verifyObject,answerClarification,reopenDeferred,nextClarification,verificationQueue,contextFor,proposeAlt,snapshot,get model(){return state.model},get packs(){return state.packs}};
})();