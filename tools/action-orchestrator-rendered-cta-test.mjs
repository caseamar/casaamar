...
const html=fs.readFileSync(path.join(root,'domain-intelligence.html'),'utf8');ok(html.includes('executeAction'),'Udfør nu handler findes');ok(html.includes('impact-actions'),'CTA container findes');ok(html.includes("['ready','deferred'].includes(x.status)"),'READY er en synlig CTA-tilstand');
const shouldRender=['ready','deferred'].includes(a.status);ok(shouldRender,'Runtime action opfylder render-betingelsen for Udfør nu');
