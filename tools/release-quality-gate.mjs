...
const mapping={platform:'platform_version',core:'core','platform-doctor':'platform_doctor','project-brain':'project_brain','decision-engine':'decision_engine','event-engine':'event_engine',diagnostics:'diagnostics_framework',configuration:'configuration_service',governance:'governance','audit-engine':'audit_engine','platform-contracts':'platform_contracts','subsystem-registry':'subsystem_registry','public-website-runtime':'public_website_runtime','repository-intelligence':'repository_intelligence','content-intelligence':'content_intelligence','ai-knowledge-graph':'ai_knowledge_graph','asset-intelligence':'asset_intelligence','experience-engine':'experience_engine','ai-workspace':'ai_workspace','content-studio':'content_studio','content-operations':'content_operations','website-configuration':'website_configuration','recommendation-engine':'recommendation_engine','release-governance':'release_governance','behaviour-governance':'behaviour_governance','engineering-governance':'engineering_governance','platform-intelligence':'platform_intelligence','learning-registry':'learning_registry','ai-service-layer':'ai_service_layer','constitution-engine':'constitution_engine','platform-integrity':'platform_integrity','data-contracts':'data_contracts','experience-context':'experience_context','capability-intelligence':'capability_intelligence','verification-governance':'verification_governance','workspace-operating-system':'workspace_operating_system','improvement-engine':'improvement_engine','action-engine':'action_engine','ai-operations-center':'ai_operations_center','observation-platform':'observation_platform','discovery-core':'discovery_core','discovery-content-intelligence':'discovery_content_intelligence','platform-orchestrator':'platform_orchestrator','capability-orchestrator':'capability_orchestrator','platform-metadata-runtime':'platform_metadata_runtime','service-container':'service_container','policy-runtime':'policy_runtime','semantic-knowledge-runtime':'semantic_knowledge_runtime'};
...
const moduleFiles={'core/casa-core.js':'core','core/casa-brain.js':'project-brain','core/casa-decision.js':'decision-engine','core/casa-events.js':'event-engine','core/casa-diagnostics.js':'diagnostics','core/casa-config.js':'configuration','core/casa-governance.js':'governance','core/casa-audit.js':'audit-engine','core/casa-contracts.js':'platform-contracts','core/casa-subsystem-registry.js':'subsystem-registry','core/casa-repository.js':'repository-intelligence','core/casa-content.js':'content-intelligence','core/casa-knowledge-graph.js':'ai-knowledge-graph','core/casa-assets.js':'asset-intelligence','core/casa-experience.js':'experience-engine','core/casa-workspace.js':'ai-workspace','core/casa-content-studio.js':'content-studio','core/casa-content-operations.js':'content-operations','core/casa-recommendations.js':'recommendation-engine','core/casa-release-governance.js':'release-governance','core/casa-behaviour-governance.js':'behaviour-governance','core/casa-engineering-governance.js':'engineering-governance','core/casa-platform-intelligence.js':'platform-intelligence','core/casa-ai-service.js':'ai-service-layer','core/casa-constitution.js':'constitution-engine','core/casa-data-contracts.js':'data-contracts','core/casa-experience-context.js':'experience-context','core/casa-capability-intelligence.js':'capability-intelligence','core/casa-verification-governance.js':'verification-governance','core/casa-workspace-os.js':'workspace-operating-system','core/casa-improvement-engine.js':'improvement-engine','core/casa-action-engine.js':'action-engine','core/casa-operations-center.js':'ai-operations-center','core/casa-observation-platform.js':'observation-platform','core/casa-discovery-core.js':'discovery-core','core/casa-discovery-content-intelligence.js':'discovery-content-intelligence','core/casa-platform-orchestrator.js':'platform-orchestrator','core/casa-capability-orchestrator.js':'capability-orchestrator','core/casa-platform-metadata.js':'platform-metadata-runtime','core/casa-service-container.js':'service-container','core/casa-policy-runtime.js':'policy-runtime','core/casa-semantic-knowledge.js':'semantic-knowledge-runtime'};
for(const [file,id] of Object.entries(moduleFiles)){const v=read(file).match(/const VERSION\s*=\s*['"]([^'"]+)['"]/)?.[1]; const rv=registry.subsystems.find(x=>x.id===id)?.version; check(v===rv,`${file} version matches registry (${v})`)}
...
// Behaviour Governance registry is mandatory for every release.
const behaviour=json('registry/behaviour-contracts.json');
check(Array.isArray(behaviour.contracts)&&behaviour.contracts.length>0,'Behaviour Governance registry is populated');
for(const contract of behaviour.contracts||[]){
 check((contract.expected_behaviour||[]).length>0,`Behaviour contract has expected scenarios (${contract.id})`);
 check((contract.forbidden_behaviour||[]).length>0,`Behaviour contract has forbidden scenarios (${contract.id})`);
 check((contract.impact_areas||[]).length>0,`Behaviour contract has impact areas (${contract.id})`);
 for(const dep of contract.dependencies||[])check(fs.existsSync(path.join(root,dep.replace(/^\//,''))),`Behaviour dependency resolves (${contract.id}: ${dep})`);
}
if(fail.length)process.exit(1);


// v148 Platform Intelligence and Learning Registry gates
const platformIntelligence=json('registry/platform-intelligence.json');
check(platformIntelligence.version===manifest.platform_intelligence.version,'Platform Intelligence registry version matches manifest');
check(platformIntelligence.overall?.because&&Array.isArray(platformIntelligence.overall?.evidence),'Platform Intelligence overall judgement is explainable');
check(platformIntelligence.overall?.next_action?.href,'Platform Intelligence attention status has a next action');
check(platformIntelligence.policy?.unknown_must_not_be_reported_as_pass===true,'Unknown results cannot be reported as PASS');
const learningRegistry=json('registry/learning-registry.json');
check(learningRegistry.version===manifest.learning_registry.version,'Learning Registry version matches manifest');
check((learningRegistry.entries||[]).length>0,'Learning Registry contains durable learnings');
for(const item of learningRegistry.entries||[]){
 check(Boolean(item.id&&item.type&&item.title&&item.root_cause&&item.learning),'Learning entry is complete ('+item.id+')');
 check(Array.isArray(item.prevention)&&item.prevention.length>0,'Learning entry has prevention ('+item.id+')');
 check(Array.isArray(item.test_evidence)&&item.test_evidence.length>0,'Learning entry has test evidence ('+item.id+')');
 for(const evidence of item.test_evidence||[])check(fs.existsSync(path.join(root,evidence)),'Learning evidence resolves ('+item.id+' -> '+evidence+')');
}
...
// v329 Governed Policy Runtime integrity gates
const policyRegistryV329=json('registry/policies.json');
const policyRuntimeV329=json('registry/policy-runtime.json');
check(manifest.policy_runtime?.version==='1.0.0','Policy Runtime manifest version is current');
check(policyRegistryV329.version==='2.0.0','Policy registry schema is operational');
check(policyRegistryV329.principles?.configuration_over_code===true,'Policies use configuration over hardcoded governance');
check(policyRegistryV329.principles?.deterministic_evaluation===true,'Policy evaluation is deterministic');
check(policyRuntimeV329.evaluation_contract?.no_dynamic_code_execution===true,'Policy Runtime forbids dynamic code execution');
check(policyRuntimeV329.evaluation_contract?.fail_closed===true,'Policy Runtime fails closed');
check((policyRegistryV329.policies||[]).every(x=>x.id&&x.version&&x.priority!==undefined&&Array.isArray(x.applies_to)&&x.when&&x.effect&&x.reason),'Policy definitions are complete');
check(fs.existsSync(path.join(root,'tools/policy-runtime-test.mjs')),'Executable Policy Runtime test is packaged');
check(fs.existsSync(path.join(root,'platform/brain/decisions/ADR-029-POLICY-RUNTIME.md')),'ADR-029 is packaged');

// v330 Semantic Knowledge Runtime integrity gates
const semanticKnowledgeV330=json('registry/semantic-knowledge.json');
check(manifest.semantic_knowledge_runtime?.version==='1.0.0','Semantic Knowledge Runtime manifest version is current');
check(semanticKnowledgeV330.principles?.claims_require_evidence===true,'Semantic knowledge requires evidence');
check(semanticKnowledgeV330.principles?.provenance_required===true,'Semantic knowledge preserves provenance');
check(semanticKnowledgeV330.principles?.confidence_is_explicit===true,'Semantic knowledge exposes confidence');
check(semanticKnowledgeV330.principles?.contradictions_are_preserved===true,'Knowledge conflicts are preserved');
check(semanticKnowledgeV330.principles?.embeddings_are_adapters_not_truth===true,'Embeddings are adapters rather than truth');
check((semanticKnowledgeV330.claims||[]).every(x=>x.id&&x.subject&&x.predicate&&x.statement&&Number.isFinite(x.confidence)&&Array.isArray(x.evidence_ids)&&x.evidence_ids.length),'Knowledge claims are complete and evidence-backed');
check(fs.existsSync(path.join(root,'tools/semantic-knowledge-runtime-test.mjs')),'Executable Semantic Knowledge Runtime test is packaged');
check(fs.existsSync(path.join(root,'platform/brain/decisions/ADR-030-SEMANTIC-KNOWLEDGE-RUNTIME.md')),'ADR-030 is packaged');
if(fail.length)process.exit(1);
