...
const failures=[];for(const id of ['report','engineering-governance','platform-intelligence','capability-intelligence','learning-registry','ai-service-layer','event-platform','data-contracts','constitution-engine','workspace-operating-system','improvement-engine','action-engine','ai-operations-center','observation-platform','content-intelligence','verification-governance']){const e=elements.get(id);if(!e)failures.push(`${id}: missing`);else if(/Indlæser/.test(e.innerHTML||e.textContent))failures.push(`${id}: loading did not settle`)}
...
if(!elements.get('observation-platform')?.innerHTML.includes('Observation Platform: v1.0.0'))failures.push('observation result missing');
if(elements.get('observation-platform')?.dataset.observationState!=='success')failures.push('observation state not success');
if(!elements.get('content-intelligence')?.innerHTML.includes('Content Intelligence Workspace: v1.0.0'))failures.push('content intelligence result missing');
...
console.log('Rendered runtime integration: 16 panels settled, 0 failed');
